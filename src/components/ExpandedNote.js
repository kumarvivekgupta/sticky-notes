...
export default function ExpandedNote({ id, noteData, handleClickModalBtn }) {
  ...
  return (
    <>
      <figure className={`flex flex-col justify-between w-96 bg-blue-500`}>
        <header className="flex justify-between items-center w-full bg-blue-700 px-2 py-1">
          <button
        ...
        {viewAsMarkDown ? (
          ...
          <section className="h-96 p-2">
            <textarea
              onChange={(e) => handleChangeNoteData(e, id)}
              className="singleNote__textArea ease-in-out duration-150 w-full h-full text-sm text-white outline-none outline-0 rounded bg-transparent p-1 focus:bg-blue-900 focus:outline-2"
              name="body"
